...
const haveVisitedSmallCaveTwice = path => {
  path.split(',').filter(el => el.toLowerCase() === el)
}

const isValidConnection = (path, connection) => {

  connection.split('-').filter(el => el.toLowerCase() === el).forEach(el => {
    if(path.includes(el) && el !== getPathEnd(path)) {
